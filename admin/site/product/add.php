<?php
	...
	$cid = $_GET['cid']; if ($cid != "") { $_SESSION['sess_cid'] = $cid; } $sess_cid = $_SESSION['sess_cid'];
	if ($_SESSION['sess_pdid'] == "") { $_SESSION['sess_pdid'] = date("ymdHis"); } $sess_pdid = $_SESSION['sess_pdid'];
	$_SESSION['sess_padmin'] = "site/product/add.php";
	
	$chk = $_POST['chk'];
	if ($chk == "1")	{
		$pdposted = date("Y-m-d H:i:s");
		...
		$pdurl = preg_replace('/[^a-zA-Z0-9\-]/', '_', $pdtitle_en);
		...
		$sqlpd = "INSERT INTO tb_product VALUES ('$sess_pdid','$sess_cid','$sess_pid','$pdurl','$pdtitle_en','$pdtitle_th','$pddetail_en','$pddetail_th','','$pdposted','0','0')";
		$resultpd = mysql_query($sqlpd,$dgz);
		if ($resultpd) { header("Location:add.php"); }
	...
	$sqlc = "select * from tb_category WHERE pid='$sess_pid' AND cid='$sess_cid'";
	...
	if (!isset($_SESSION['random_key']) || strlen($_SESSION['random_key'])==0){
		$_SESSION['random_key'] = $sess_pid."_".$sess_cid."".$sess_pdid; //assign the timestamp to the session variable
		$_SESSION['user_file_ext']= "";
	}
	
	// CONSTANTS
	$upload_dir = "../../resources/product"; 	// The directory for the images to be saved in
	...
	$thumb_width = "122";						// Width of thumbnail image
	$thumb_height = "122";						// Height of thumbnail image
...
<body>
	...
	<p id="nav">
		<a href="../category/update.php?pid=<?php echo $sess_pid;?>&amp;cid=<?php echo $sess_cid;?>"><?php echo $c[ctitle_en];?></a> &nbsp;&gt;&nbsp; 
        <a href="index.php?pid=<?php echo $sess_pid;?>&amp;cid=<?php echo $sess_cid;?>">All Products</a> &nbsp;&gt;&nbsp; 
		<strong>Post new Product</strong>
	</p>
	...
	<div id="content">
		...
		if (strlen($large_photo_exists) > 0 && strlen($thumb_photo_exists) > 0)	{
			...
			$sqlpd = "UPDATE tb_product SET pdpic='$thname' WHERE pdid='$sess_pdid' AND pid='$sess_pid' AND cid='$sess_cid'";
			$resultpd = mysql_query($sqlpd,$dgz);
			if ($resultpd) {
				...
				if ($_SESSION['sess_pdid'] == "" and $_SESSION['sess_pid'] == "" and $_SESSION['sess_cid'] == "") { print "<meta http-equiv=\"refresh\"content=\"0; URL=index.php?pid=$pid&cid=$cid\">"; }
			} else {
		...
		<?php } ?>
			...
			function check()	{
				if (document.photo.pdtitle_en.value == "")	{
					alert("Please fill in title");
					document.photo.pdtitle_en.focus();
					return false;
				}
				if (document.photo.pdtitle_th.value == "")	{
					alert("Please fill in title");
					document.photo.pdtitle_th.focus();
					return false;
				}
            ...
            <h3><img src="../../images/tools/add.png" alt="" />&nbsp; Post new Product</h3>	
			<table cellpadding="0" cellspacing="0" class="contentinfo">
                ...
                <tr>
                    ...
                    <td style="padding-top:25px; width:740px;"><input name="pdtitle_en" type="text" class="box" value="" size="" style="width:700px;" /></td>
                </tr>
                <tr>
                    <td>Detail / Info :</td>
                    <td><textarea name="pddetail_en" class="box" rows="10" style="width:700px;"></textarea><script type="text/javascript">CKEDITOR.replace('pddetail_en');</script></td>
                </tr>
                <tr>
                    <td>&nbsp;</td>
                ...
                <tr>
                    <td style="padding-top:25px; width:160px;">Title :</td>
                    <td style="padding-top:25px; width:740px;"><input name="pdtitle_th" type="text" class="box" value="" size="" style="width:700px;" /></td>
                </tr>
                <tr>
                    <td>Detail / Info :</td>
                    <td><textarea name="pddetail_th" class="box" rows="10" style="width:700px;"></textarea><script type="text/javascript">CKEDITOR.replace('pddetail_th');</script></td>
                </tr>
                <tr>
                    <td>&nbsp;</td>
                    <td class="tip">^ When copying text from anywhere, please ensure you click either the "Paste as plain text" button or "Paste from Word" button. (Which are found at the top of the textarea editor, brown button). This is to ensure no unwanted code will damage the layout structure.</td>
